...
sub vcl_recv {
    ...
    if (req.http.Accept-Encoding) {
        if (req.url ~ "\.(jpg|jpeg|png|gif|gz|tgz|bz2|tbz|mp3|ogg|swf|mp4|flv|woff)$") {
    ...
    if (req.url ~ "^[^?]*\.(css|js|htc|xml|txt|swf|flv|pdf|gif|jpe?g|png|ico|woff)$") {
...
sub vcl_backend_response {
    ...
    if (beresp.ttl > 0s) {
        /* Remove Expires from backend, its not long enough */
	...
	if (bereq.url ~ "^[^?]+\.(jpg|jpeg|png|gif|ico|gz|tgz|bz2|tbz|mp3|ogg|swf|mp4|flv|woff)(\?.*)?$") {
